...
/// Quiz view for the Gemini cloud API.
/// Mirrors the existing QuizView UI but uses GeminiQuizViewModel.
struct GeminiQuizView: View {
  @Bindable var quizViewModel: GeminiQuizViewModel
  ...
  private func loadingView(_ text: String) -> some View {
    VStack(spacing: 24) {
      ...
      VStack(spacing: 8) {
        Text("Gemini is drafting your quiz…")
          ...
          if quizViewModel.showExplanation {
            ...
            .background(
              RoundedRectangle(cornerRadius: 16)
                .fill(Color(hex: "34A853").opacity(0.08))
                .overlay(
                  RoundedRectangle(cornerRadius: 16)
                    .stroke(Color(hex: "34A853").opacity(0.15), lineWidth: 1)
                )
            )
            .padding(.horizontal, 20)
          ...
          if quizViewModel.showExplanation {
            ...
            if isLastCurrentlyGenerated && hasMoreToGenerate {
              HStack(spacing: 8) {
                ...
                Text("Gemini is writing question \(currentNum + 1)...")
                  .font(.system(size: 14, weight: .semibold))
                  .foregroundStyle(Color(hex: "34A853"))
              }
              ...
              .background(
                RoundedRectangle(cornerRadius: 14)
                  .fill(Color(hex: "34A853").opacity(0.05))
                  .overlay(
                    RoundedRectangle(cornerRadius: 14)
                      .stroke(Color(hex: "34A853").opacity(0.2), lineWidth: 1.5)
                  )
              )
              .padding(.horizontal, 20)
              .padding(.bottom, 24)
    ...
    VStack(spacing: 24) {
      ...
      ZStack {
        Circle()
          .fill(
            LinearGradient(
              colors: [Color(hex: "4285F4").opacity(0.15), Color(hex: "34A853").opacity(0.15)],
      ...
      Text(scoreLabel)
        ...
        .padding(.vertical, 8)
        .background(
          Capsule().fill(Color(hex: "34A853").opacity(0.08))
        )
